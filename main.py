...
class BotConfig:
    def __init__(self):
        ...
        # Welcome/Leave/Invite Channels & Messages (Owner Configurable)
        self.WELCOME_CHANNEL_ID = None
        self.LEAVE_CHANNEL_ID = None
        self.INVITE_CHANNEL_ID = None  # NEW - for invite logs
        
        self.WELCOME_MSG = "Welcome {user} to {servername}! You are member #{count}."
        self.LEAVE_MSG = "Goodbye {user}! You were member #{count}."
        self.INVITE_MSG = "{user} has been invited by {inviter} and has now {invites} invites!"  # NEW
        
        self.WELCOME_ENABLED = False
        self.LEAVE_ENABLED = False
        self.INVITE_LOG_ENABLED = False  # NEW
        ...
        try:
            if os.path.exists('bot_config.json'):
                with open('bot_config.json', 'r') as f:
                    ...
                    self.INVITE_CHANNEL_ID = data.get('invite_channel_id', self.INVITE_CHANNEL_ID)
                    ...
                    self.INVITE_LOG_ENABLED = data.get('invite_log_enabled', self.INVITE_LOG_ENABLED)
                    ...
                    self.FAKE_INVITE_DAYS = data.get('fake_invite_days', self.FAKE_INVITE_DAYS)
        except: pass
    ...
    def save_config(self):
        try:
            data = {
                ...
                'invite_channel_id': self.INVITE_CHANNEL_ID,
                ...
                'invite_log_enabled': self.INVITE_LOG_ENABLED,
                ...
                'fake_invite_days': self.FAKE_INVITE_DAYS
            }
...
class DatabaseManager:
    ...
    async def initialize_tables(self):
        async with self.pool.acquire() as conn:
            ...
            await conn.execute('''CREATE TABLE IF NOT EXISTS invites (
                user_id BIGINT,
                guild_id BIGINT,
                inviter_id BIGINT,
                ...
                left_at TIMESTAMP,
                PRIMARY KEY(user_id, guild_id, joined_at)
            )''')
    ...
    async def get_invites(self, inviter_id: int, guild_id: int):
        async with self.pool.acquire() as conn:
            result = await conn.fetchrow('''
                SELECT 
                    COUNT(*) FILTER (WHERE left_at IS NULL AND is_fake = FALSE AND is_rejoin = FALSE) as total,
                    COUNT(*) FILTER (WHERE left_at IS NULL AND is_fake = FALSE AND is_rejoin = FALSE) as real,
                    COUNT(*) FILTER (WHERE left_at IS NOT NULL) as left,
                    COUNT(*) FILTER (WHERE is_fake = TRUE) as fake,
                    COUNT(*) FILTER (WHERE is_rejoin = TRUE) as rejoin
                FROM invites 
    ...
    async def get_leaderboard(self, guild_id: int, limit: int = 10):
        async with self.pool.acquire() as conn:
            results = await conn.fetch('''
                SELECT inviter_id, COUNT(*) FILTER (WHERE left_at IS NULL AND is_fake = FALSE AND is_rejoin = FALSE) as real
                FROM invites WHERE guild_id = $1
                GROUP BY inviter_id 
                HAVING COUNT(*) FILTER (WHERE left_at IS NULL AND is_fake = FALSE AND is_rejoin = FALSE) > 0
                ORDER BY real DESC LIMIT $2
            ''', guild_id, limit)
            return results
    ...
    async def track_message(self, user_id: int, guild_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute('''
                INSERT INTO messages (user_id, guild_id, message_count, last_message_at)
                VALUES ($1, $2, 1, CURRENT_TIMESTAMP)
                ON CONFLICT (user_id, guild_id) 
                DO UPDATE SET message_count = messages.message_count + 1, last_message_at = CURRENT_TIMESTAMP
            ''', user_id, guild_id)
...
def format_msg(template: str, user: discord.Member, inviter: Optional[discord.Member] = None, count: int = 0, invite_count: int = 0) -> str:
    """Format welcome/leave/invite messages"""
...
async def on_ready():
    ...
    logger.info("="*60)
    logger.info("🚀 ULTIMATE BOT - PERFECT VERSION")
...
async def on_member_join(member):
    ...
    # WELCOME MESSAGE (configurable by owner)
...
async def on_member_remove(member):
    ...
    # LEAVE MESSAGE (configurable by owner)
...
async def botconfig(ctx):
    ...
    embed.add_field(name="Commands", value=
        ...
        f"`{config.PREFIX}config reset` - Reset", inline=False)
    await ctx.send(embed=embed)
...
# ==================== ENHANCED INVITE COMMANDS (FALCON STYLE) ====================
...
@is_owner()
async def setjoinchannel_cmd(ctx, channel: discord.TextChannel = None):
    if channel is None:
        config.JOIN_LOG_ENABLED = False
        config.JOIN_CHANNEL_ID = None
        config.save_config()
        return await send_embed(ctx, "❌ Join Logs Disabled", "Join logging turned off!", discord.Color.red())
    
    config.JOIN_CHANNEL_ID = channel.id
    config.JOIN_LOG_ENABLED = True
    config.save_config()
    await send_embed(ctx, "✅ Join Channel Set", f"Join logs → {channel.mention}", discord.Color.green())

@bot.command(name="lbi", aliases=["leaderboardinvites"])
async def lbi_cmd(ctx):
    lb = await db_manager.get_leaderboard(ctx.guild.id, 10)
    
    if not lb:
        return await send_embed(ctx, "📊 Leaderboard Empty", "No invites tracked yet!", discord.Color.orange())
...
# ==================== OWNER CONFIG - WELCOME/LEAVE/INVITE (OWNER ONLY) ====================
...
async def setinvitechannel_cmd(ctx, channel: discord.TextChannel = None):
    ...
    config.save_config()
    await send_embed(ctx, "✅ Invite Channel Set", f"Invite logs → {channel.mention}\n\nFormat: `@user has been invited by @inviter and has now X invites!`", discord.Color.green())
...
async def setwelcomemsg_cmd(ctx, *, message: str):
    ...
    config.WELCOME_ENABLED = True
    config.save_config()
    await send_embed(ctx, "✅ Welcome Message Set", f"**Message:** {message}\n\n**Variables:**\n`{{user}}` `{{username}}` `{{servername}}` `{{count}}`", discord.Color.green())
